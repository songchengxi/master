...
<c:forEach items="${dataRuleList}" var="dataRule">
    ...
    <c:if test="${fn:contains(dataRulecodes, dataRule.id)}">
        <input style="width: 20px;" type="checkbox" name="operationCheckbox1" value="${dataRule.id}"
               checked="checked"/>${dataRule.ruleName}
    </c:if>
    <c:if test="${!fn:contains(dataRulecodes, dataRule.id)}">
...
<script type="text/javascript">
    function submitDataRule() {
        ...
        doSubmit("companyRoleController.do?updateDataRule&functionId=" + functionId + "&roleId=" + roleId + "&dataRulecodes=" + dataRulecodes);
    }
</script>
